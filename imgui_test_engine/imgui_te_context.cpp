...
void ImGuiTestContext::SetRef(ImGuiTestRef ref)
{
    IMGUI_TEST_CONTEXT_REGISTER_DEPTH(this);
    if (ActiveFunc != ImGuiTestActiveFunc_GuiFunc)
        LogDebug("SetRef '%s' 0x%08X", ref.Path.length() > 0 ? ref.Path.c_str() : "nullptr", ref.ID);
...
void ImGuiTestContext::CaptureSetExtension(const char* ext)
{
    ...
    {
        ImFormatString(args->InOutputFile, IM_COUNTOF(args->InOutputFile), "output/captures/%s_%04d%s", Test->Name.c_str(), CaptureCounter, ext);
        CaptureCounter++;
    }
    else
    {
...
ImGuiTestItemInfo ImGuiTestContext::ItemInfo(ImGuiTestRef ref, ImGuiTestOpFlags flags)
{
    ...
    if (const char* p = ref.Path.length() > 0 ? strstr(ref.Path.c_str(), "**/") : nullptr)
    {
        // Wildcard matching
...
ImGuiTestItemInfo ImGuiTestContext::ItemInfoOpenFullPath(ImGuiTestRef ref, ImGuiTestOpFlags flags)
{
    ...
    for (const char* next_path = nullptr; path[0] != 0; path = next_path)
    {
        ...
        Str128 parent_path;
        parent_path.set(ref.Path.c_str(), parent_end);
...
ImGuiTestItemInfo ImGuiTestContext::WindowInfo(ImGuiTestRef ref, ImGuiTestOpFlags flags)
{
    ...
    LogDebug("WindowInfo: by path: '%s'", ref.Path.length() > 0 ? ref.Path.c_str() : "nullptr");
    ...
    while (*current || window == nullptr)
    {
        ...
        Str128 part_name;
        if (window == nullptr && RefID != 0 && strncmp(ref.Path.c_str(), "//", 2) != 0)
        {
...
void    ImGuiTestContext::MenuAction(ImGuiTestAction action, ImGuiTestRef ref)
{
    ...
    IM_ASSERT(ref.Path.c_str() != nullptr);
